...
int rdata=0; //receiving from root
MPI_Recv(&rdata, 1, MPI_INT, 0, rank, MPI_COMM_WORLD,&st);
printf("\n process %d gathered: %d ",rank,rdata);
	...
	for(int i=0;i<8;i++)
		...
		printf("%d  ",total);
	for(int i=1;i<8;i++)
 MPI_Isend(&total, 1, MPI_INT, i, i, MPI_COMM_WORLD,&req);   //sending to all processes
     MPI_Wait(&req,&st);
  }
